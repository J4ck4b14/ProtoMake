...
export const Perception2D: ComponentDefinition<PerceptionData> = {
  ...
  defaults: () => ({ target: '', range: 450, fovDegrees: 90, illuminationThreshold: 0.6, debug: true }),
  inspector: [
    { path: 'target', label: 'Target', kind: 'entity' },
    { path: 'range', label: 'View range', kind: 'number', min: 0, step: 10 },
    { path: 'fovDegrees', label: 'View angle', kind: 'number', min: 1, max: 360, step: 1 },
    { path: 'illuminationThreshold', label: 'Light threshold', kind: 'number', min: 0, max: 1, step: 0.05 },
